...
export default {
  ...
  methods: {
    ...
    editItem(item) {
      ...
      this.editedItem.password = '' // No mostrar la contraseña actual
      this.dialog = true
    },
    ...
    async deleteItemConfirm() {
      try {
        await UsersService.deleteUser(this.editedItem.id);
    ...
    async save() {
      try {
        if (this.editedIndex > -1) {
          const response = await UsersService.updateUser(this.editedItem.id, this.editedItem);
          Object.assign(this.users[this.editedIndex], response.data);
        } else {
          const response = await UsersService.createUser(this.editedItem);
